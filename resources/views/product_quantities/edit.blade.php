                ...
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('product_quantities.store') }}">
                            ...
                            @if(isset($productQuantity)) <!-- Check if you are in "edit" mode -->
                            ...
                            <div class="form-group">
                                ...
                                <select name="company_id" id="company_id" class="form-control">
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" {{ (isset($productQuantity) && $productQuantity->company_id == $company->id) ? 'selected' : '' }}>
                                            {{ $company->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group" id="quantity-input" style="display: none;">
                                ...
                                <input type="number" name="quantity" class="form-control" placeholder="Enter Quantity" required value="{{ old('quantity', (isset($productQuantity) ? $productQuantity->quantity : '')) }}">
                                @error('quantity')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>

                            <button type="submit" class="btn btn-primary" id="add-quantity-button" style="display: none;">{{ isset($productQuantity) ? 'Update Quantity' : 'Add Quantity' }}</button>
                        </form>
                    </div>
                </div>
            </div>
